...
class ServerNet(QWidget, Ui_GameBoard):
    ...
    def listentoclient(self, client,address): #This is actioned for each client connection
        ...
        while True:
            try:
                ...
                if msg[0:6] == "Accuse":
                    self.checkAccusation(msg)
                
                if msg[0:8] == "Suggest":
                    self.disproveSuggestion(msg)
    ...
    def options(self):
        ''' Not sure if we need an options function on the server. '''

    def leaveGame(self, incomingmsg):
        msg = incomingmsg[5:]
        playerInfo = msg.split(',')
        ''' Incoming message will contain player type and number and will either
            end game, or remove said player. '''

    def checkAccusation(self, incomingmsg):
        msg = incomingmsg[6:]
        accusation = msg.split(',')
        ''' Need to know where is the final evidence to test against.
            Also, need to know where to store result.'''
        result = False
        if (accusation[0].lower() == "mr. green" and accusation[1].lower() == "revolver" and accusation[2].lower() == "study"):
            outMSG = "Result" + "win" + "," + accusation[0] + "," + accusation[1] + "," + accusation[2]
            self.sendmsgtoclients(outMSG)
        else:
            outMSG = "Result" + "lost" + ","
            self.sendmsgtoclients(outMSG)

    def disproveSuggestion(self, incomingmsg):
        ''' Appending Zack's code. Trying to integrate.'''
        ''' Need to remove the print lines and ... '''
    ...
    def nextTurn(self, incomingmsg):
        ''' Send a message to the client so can display your turn dialog'''
